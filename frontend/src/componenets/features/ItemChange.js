...
const ItemChange = ({ item, onEdit, onDelete }) => {
    ...
    const handlePriceChange = event => {
        ...
        setPriceError(!validator.isNumeric(event.target.value) || validator.isEmpty(event.target.value) || event.target.value < 0);
    }
    ...
    const handleEdit = item => {
        ...
        if (itemDetails.price > 200) {
            ...
            Swal.fire({
                title: 'Error',
                text: 'Price for an item cannot be more than 200!',
                icon: 'error',
                confirmButtonText: 'OK'
            });
