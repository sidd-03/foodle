...
const OrderCard = ({ buyer, order, item, vendor, unit, set_unit }) => {
    const [rating, setRating] = useState(order.rating);

    const matches = useMediaQuery('(min-width:480px)');

    const handleRatingChange = value => {
        axios.patch(`http://localhost:5000/api/items/update_rating`, {
                item_id: item._id,
                order_id: order._id,
                rating: value
            }, {
            ...
            .then(res => {
                ...
                set_unit({
                    ...unit,
                    items: unit.items.map(i => (i._id === item._id ? res.data.item : i)),
                    orders: unit.orders.map(o => (o._id === order._id ? res.data.order : o))
                });
    ...
    const handleStatusUpdate = () => {
        axios.patch(`http://localhost:5000/api/orders/update_state`, {
                order_id: order._id,
            }, {
                headers: {
                    authorization: localStorage.getItem('token')
    ...
    const handleRejection = () => {
        axios.patch(`http://localhost:5000/api/orders/reject`, {
                order_id: order._id,
                buyer_id: buyer._id,
            }, {
                headers: {
